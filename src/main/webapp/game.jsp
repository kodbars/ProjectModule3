...
<c:if test="${showRestartButton}">
    <form action="${pageContext.request.contextPath}/game?reset=true" method="post">
        <button type="submit" name="answer" value="restart" class="btn restart"><Span>Попробовать еще раз</Span></button>
    </form>
</c:if>
...
<c:if test="${not empty victoryImage}">
    <img src="${victoryImage}" alt="Victory Image" class="images">
</c:if>
